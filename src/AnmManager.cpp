...

{
  ...
  g_PrimitivesToDrawVertexBuf[2].w = 1.0f;
  g_PrimitivesToDrawVertexBuf[1].w = 1.0f;
  g_PrimitivesToDrawVertexBuf[0].w = 1.0f;
  ...
  g_PrimitivesToDrawNoVertexBuf[2].w = 1.0f;
  g_PrimitivesToDrawNoVertexBuf[1].w = 1.0f;
  g_PrimitivesToDrawNoVertexBuf[0].w = 1.0f;
...

{
  ...
  g_PrimitivesToDrawUnknown[1].textureUV =
      this->vertexBufferContents[1].textureUV;
  g_PrimitivesToDrawUnknown[2].textureUV =
      this->vertexBufferContents[2].textureUV;
  g_PrimitivesToDrawUnknown[3].textureUV =
      this->vertexBufferContents[3].textureUV;
...

{
  ...
  if (srcData == NULL) {
    ...
    if (D3DXCreateTextureFromFileInMemoryEx(
            ...
            g_TextureFormatD3D8Mapping[formatIdx], D3DPOOL_MANAGED, 3,
            0xffffffff, colorKey, NULL, NULL,
...

{
  ...
  i32 startIdx = anmIdx;
  AnmRawEntry *local_14 = (AnmRawEntry *)FileSystem::OpenFile(path, 0);
  u8 uVar1 = 1;
  if (local_14 == NULL) {
    g_GameErrorContext.Fatal(
        "アニメが読み込めません。データが失われてるか壊れています\r\n");
    return ZUN_ERROR;
  } else {
    while (true) {
      res = LoadAnm(anmIdx, local_14, spriteIdxOffset, uVar1);
      if (res < 0) {
        this->anmFiles[startIdx].childCount = anmIdx - startIdx;
        return res;
      }
      anmIdx = anmIdx + 1;
      if (local_14->nextOffset == 0) {
        this->anmFiles[startIdx].childCount = anmIdx - startIdx;
        return ZUN_SUCCESS;
      }
      local_14 = (AnmRawEntry *)((u8 *)local_14 + local_14->nextOffset);
      uVar1 = 0;
...
i32 AnmManager::LoadAnm(i32 textureIdx, AnmRawEntry *rawEntry,
                        i32 spriteIdxOffset, u8 param_4)

{
  ...
  if (rawEntry == NULL) {
    g_GameErrorContext.Fatal(
        "アニメが読み込めません。データが失われてるか壊れています\r\n");
    return ZUN_ERROR;
  } else if (textureIdx < 0x32) {
    ReleaseAnm(textureIdx);
    data = rawEntry;
    if (rawEntry->version == 2) {
      rawEntry->textureIdx = textureIdx;
      rawEntry->ownsMemory = param_4;
      if (rawEntry->hasData == 0) {
        name = (char *)((u8 *)rawEntry + rawEntry->nameOffset);
        if (*name == '@') {
          CreateEmptyTexture(rawEntry->textureIdx, rawEntry->width,
                             rawEntry->height, rawEntry->format);
        } else {
          if (LoadTexture(rawEntry->textureIdx, name, rawEntry->format,
                          rawEntry->color_key) != ZUN_SUCCESS) {
            g_GameErrorContext.Fatal(
                "テクスチャ %s "
        ...
        if (LoadTextureEmbedded(
                rawEntry->textureIdx,
                (ZunImageInfoEmbedded *)((u8 *)rawEntry +
                                         rawEntry->textureOffset),
                rawEntry->format) != ZUN_SUCCESS) {
          g_GameErrorContext.Fatal(
              "テクスチャが読み込めません。データが失われて"
              "るか壊れています\r\n");
          return ZUN_ERROR;
        }
      }
      ...
      for (i = 0; i < data->numSprites; i = i + 1) {
        ...
        loadedSprite.startPixelInclusive =
            loadedSprite.cols * rawSprite->offset;
        ...
        if (0x9ff < rawSprite->id + spriteIdxOffset) {
          g_GameErrorContext.Fatal(
              "スプライトが格納できません。テーブルが不足しています\r\n");
          return ZUN_ERROR;
        }
        LoadSprite(rawSprite->id + spriteIdxOffset, &loadedSprite);
        curSprite = curSprite + 1;
      }
      for (i = 0; i < data->numScripts; i = i + 1) {
        if (0x9ff < *curSprite + spriteIdxOffset) {
          g_GameErrorContext.Fatal(
              "アニメが格納できません。テーブルが不足しています\r\n");
          return ZUN_ERROR;
        }
...

{
  ...
  i32 local_14;
  i32 i;
  i32 sprite_index_offset;

  if (((-1 < anmIdx) && ((u32)anmIdx < 0x32)) &&
      (this->anmFiles[anmIdx].raw != NULL)) {
    after_hdr = (this->anmFiles[anmIdx].raw)->spriteOffsets;
    sprite_index_offset = this->anmFiles[anmIdx].spriteIndexOffset;
    _Memory = this->anmFiles[anmIdx].raw;
    local_14 = anmIdx;
    for (i = 1; local_14 = local_14 + 1, i < this->anmFiles[anmIdx].childCount;
         i = i + 1) {
      ReleaseAnm(local_14);
    }
    for (i = 0; i < _Memory->numSprites; i = i + 1) {
      i32 *piVar2 = (i32 *)((u8 *)_Memory + *after_hdr);
      AnmLoadedSprite *pAVar3 = this->sprites + *piVar2 + sprite_index_offset;
      memset(pAVar3, 0, sizeof(AnmLoadedSprite));
      this->sprites[*piVar2 + sprite_index_offset].sourceFileIndex = -1;
      after_hdr = after_hdr + 1;
    }
    for (i = 0; i < _Memory->numScripts; i = i + 1) {
      this->scripts[*after_hdr + sprite_index_offset] = NULL;
      this->spriteIndices[*after_hdr + sprite_index_offset] = 0;
      after_hdr = after_hdr + 2;
    }
    this->anmFiles[anmIdx].spriteIndexOffset = 0;
    ReleaseTexture(_Memory->textureIdx);
    if (_Memory->ownsMemory != 0) {
      free(_Memory);
    }
    this->anmFiles[anmIdx].raw = NULL;
...
{
  if ((-1 < textureIdx) && ((u32)textureIdx < 0x108)) {
...

{
  ...
  this->loadedSpriteCount = this->loadedSpriteCount + 1;
...
ZunResult AnmManager::SetActiveSprite(AnmVm *vm, i32 param_2)

{
  if (this->sprites[param_2].sourceFileIndex < 0) {
    return ZUN_ERROR;
  } else {
    vm->activeSpriteIdx = (i16)param_2;
    vm->sprite = &this->sprites[param_2];
...

{
  if (this->spritesToDraw != 0) {
    ...
    g_Supervisor.d3dDevice->DrawPrimitiveUP(
        D3DPT_TRIANGLELIST, this->spritesToDraw << 1,
        this->vertexBufferStartPtr, sizeof(VertexTex1DiffuseXyzrwh));
    this->vertexBufferStartPtr = this->vertexBufferCurPtr;
    this->spritesToDraw = 0;
...

{
  ...
  f32 scaleY = vm->sprite->cols;
  u32 fontHeight = vm->fontHeight;
  u32 textureHeight = vm->sprite->textureHeight;
  u32 textureWidth = vm->sprite->textureWidth;
  u32 y = vm->sprite->startPixelInclusive.y;
  u32 x = vm->sprite->startPixelInclusive.x;

  this->DrawTextToSprite(vm->sprite->sourceFileIndex, x, y, textureWidth,
                         textureHeight, fontWidth, fontHeight, textColor,
                         outlineType, (char *)" ", scaleY, scaleX);

  u32 len = strlen(buf);
  u32 local_c = (vm->sprite->widthPx * vm->sprite->cols +
                 vm->sprite->startPixelInclusive.x) -
                ((f32)len * (f32)fontWidth * vm->sprite->cols) / 2.0f;

  this->DrawTextToSprite(vm->sprite->sourceFileIndex, local_c, y, textureWidth,
                         textureHeight, fontWidth, fontHeight, textColor,
                         outlineType, buf, scaleY, scaleX);
...

{
  ...
  f32 scaleX = vm->sprite->rows;
  f32 scaleY = vm->sprite->cols;
  u32 fontHeight = (u32)vm->fontHeight;
  u32 textureHeight = (u32)vm->sprite->textureHeight;
  u32 textureWidth = (u32)vm->sprite->textureWidth;
  u32 y = (u32)vm->sprite->startPixelInclusive.y;
  u32 x = (u32)vm->sprite->startPixelInclusive.x;
  ...
  u32 len = strlen(buf);
  u32 local_c = (u32)(((vm->sprite->widthPx * vm->sprite->cols) / 2.0f +
                       vm->sprite->startPixelInclusive.x) -
                      ((f32)len * (f32)fontWidth * vm->sprite->cols) / 4.0f);
...
void AnmManager::CopyTexture(i32 param1, i32 param2, RECT *param3,
                             RECT *param4) {
  if (this->textures[param1] != NULL && this->textures[param2] != NULL) {
    this->Flush();
...
ZunResult AnmManager::UpdateTrail(
    AnmVm *vm, VertexTex1DiffuseXyzrwh *vertices, i32 count)

{
  ...
  if (count < 3) {
    ...
    local_10 = vertices;
    local_18 = vm->sprite->uvEnd.x +
               vm->uvScrollPos.x;
    for (local_c = 0; local_c < count; local_c += 2) {
      local_10->textureUV.x = local_18;
      local_10->textureUV.y = vm->sprite->uvStart.y +
                                vm->uvScrollPos.y;
    ...
    for (local_c = 1; local_c < count; local_c += 2) {
      local_10->textureUV.x = local_18;
      local_10->textureUV.y = vm->sprite->uvEnd.y +
                                vm->uvScrollPos.y;
      local_10->color = vm->color;
      local_10->w = 1.0f;
      local_10 += 2;
