...

{
  ...
  if (-1 < shtEntry->soundIdx) {
    g_SoundPlayer.PlaySoundByIdx((SoundIdx)shtEntry->soundIdx);
  }
...

{
  ...
  g_GameManager.cherryPlus = g_GameManager.globals->cherryStart;
  g_SoundPlayer.PlaySoundByIdx(SOUND_BORDER_BREAK);
...

{
  ...
  if (this->playerState != PLAYER_STATE_SPAWNING) {
    ...
    if (this->playerState != PLAYER_STATE_INVULNERABLE) {
      ...
      g_SoundPlayer.PlaySoundByIdx(SOUND_BORDER_ACTIVATE);
      g_SoundPlayer.PlaySoundByIdx(SOUND_BORDER_ACTIVATE2);
...

{
  ...
  effect->vm.interpEndTimes[4].Initialize(0x1e);
  ...
  effect->vm.interpStartTimes[2].Initialize(0);
  effect->vm.interpEndTimes[2].Initialize(0x1e);
  ...
  this->borderInvulnerabilityTime = 0x28;
  g_GameManager.cherryPlus = g_GameManager.globals->cherryStart;
  SpawnBombEffect(&this->positionCenter, 32.0f, 16.0f, 0x32, ITEM_STAR);
  local_10 = -ZUN_PI;
  for (local_c = 0; local_c < 0x20; local_c += 1) {
...

{
  ...
  fVar2 = this->positionCenter.x - param_2->x;
  fVar1 = this->positionCenter.y - param_2->y;
...

{
  ...
  (g_Player.calcChain)->addedCallback = (ChainLifecycleCallback)AddedCallback;
  (g_Player.calcChain)->deletedCallback =
      (ChainLifecycleCallback)DeletedCallback;
  if (g_Chain.AddToCalcChain(g_Player.calcChain, 8) == 0) {
    g_Chain.AddToDrawChain(g_Player.drawChain1, 6);
    g_Chain.AddToDrawChain(g_Player.drawChain2, 8);
    return ZUN_SUCCESS;
  } else {
    return ZUN_ERROR;
  }
