...

{
  ...
  while (true) {
    ...
    g_SoundPlayer.PlaySoundByIdx((i % 2) + 2);
    if (-1 < enemy->deathAnm1) {
  ...
  LAB_00421da7:
    if (enemy->damageTintTimer == 0) {
      if (local_3c == 0) {
        ...
        g_SoundPlayer.PlaySoundByIdx(SOUND_20);
...

{
  ...
  g_EnemyManager.stgEnmAnmFilename = stgEnm1;
  g_EnemyManager.stgEnm2AnmFilename = stgEnm2;
  if (g_Chain.AddToCalcChain(&g_EnemyManagerCalcChain, 10) == 0) {
    ...
    g_EnemyManagerDrawChain1.arg = &g_EnemyManager;
    if (g_Chain.AddToDrawChain(&g_EnemyManagerDrawChain1, 5) == ZUN_SUCCESS) {
      ...
      g_EnemyManagerDrawChain2.arg = &g_EnemyManager;
